...
impl HoeffdingTree {
    ...
    fn new_learning_node_with_values(
        ...
        match self.leaf_prediction_option {
            ...
            LeafPredictionOption::NaiveBayes => {
                Rc::new(RefCell::new(LearningNodeNB::new(initial_class_observations)))
            }
    ...
    pub fn estimate_model_byte_sizes(&self) {
        ...
        for found in &learning_nodes {
            if let Some(node) = found.get_node() {
                let guard = node.borrow();
                if let Some(active) = guard.as_any().downcast_ref::<ActiveLearningNode>() {
                } else if let Some(inactive) = guard.as_any().downcast_ref::<InactiveLearningNode>()
                {
                }
            }
        }
