...
pub struct SplitNode {
    ...
    children: Vec<Option<Arc<RwLock<dyn Node>>>>,
}
...
impl SplitNode {
    ...
    pub fn set_child(&mut self, index: usize, child: Arc<RwLock<dyn Node>>) {
...
impl Node for SplitNode {
        ...
        Self: Sized,
    {
        ...
        if let Some(idx) = child_index {
            if let Some(child_arc) = self_arc.read().unwrap().get_child(idx) {
        ...
        FoundNode::new(
            Some(self_arc.clone() as Arc<RwLock<dyn Node>>),
            parent,
            parent_branch,
        )
    }
...
mod tests {
    ...
    fn test_set_and_get_child_with_real_node() {
        ...
        let leaf = Arc::new(RwLock::new(InactiveLearningNode::new(vec![5.0, 5.0])));
        node.set_child(0, leaf.clone());

        let retrieved = node.get_child(0).unwrap();
        let guard = retrieved.read().unwrap();
    ...
    fn test_distribution_sum_with_real_nodes() {
        ...
        let leaf1 = Arc::new(RwLock::new(InactiveLearningNode::new(vec![2.0, 3.0])));
        let leaf2 = Arc::new(RwLock::new(InactiveLearningNode::new(vec![4.0, 1.0])));
    ...
    fn test_filter_instance_to_leaf_routes_to_real_node() {
        let test = Box::new(DummyTest { branch: Some(0) });
        let test = Box::new(DummyTest { branch: Some(0) });
        let node_arc = Arc::new(RwLock::new(SplitNode::new(test, vec![1.0, 2.0], Some(1))));

        let leaf = Arc::new(RwLock::new(InactiveLearningNode::new(vec![3.0, 7.0])));
        node_arc.write().unwrap().set_child(0, leaf.clone());
        ...
        let found_guard = found_node_arc.read().unwrap();
